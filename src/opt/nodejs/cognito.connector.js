...
const refresh = curry((_refreshSession, token) => {
    ...
    return new Promise((resolve, reject) => {
        _refreshSession(token, (err, session) => {
            if (err)
                reject(err)
            else {
...
const refreshTokenSafe = curry((getCognitoRefreshToken, getCognitoUser, username, token) => {
    try {
...
const getTokens = curry((getAccessToken, getRefreshToken, getIdToken) => {
    ...
    return createTokensObject(accessToken, refreshToken, tokenId)
})
